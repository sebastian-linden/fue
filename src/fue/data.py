...
try:
    ...
    from openmeteoclient import OpenMeteoClient  # When run directly
...
class Data():
    ...
    def read_raw(self, path=None):
        ...
        try:
            ...
            raise FileNotFoundError("forecasts.csv was not found")
        return None
    ...
    def generate_dataset(self, location_name="aachen"):
        ...
        # 2. Separate forecasts from measurements
        forecast_split = local_raw[(local_raw["forecast_for"] - local_raw["forecasted_on"]).dt.total_seconds() > 12*60*60]
        measured_split = local_raw[(local_raw["forecast_for"] - local_raw["forecasted_on"]).dt.total_seconds() <= 12*60*60]
        ...
        dataset["delta_days"] = (forecast_split["forecast_for"] - forecast_split["forecasted_on"]).dt.total_seconds()/(60*60*24)
        ...
        for row in measured_split.iterrows():
            ...
            for var in self.weather_variables:
                dataset.loc[match_condition, f"abs_diff__{var}"] = (dataset[match_condition][var]-measurement[var]).abs()
